...
"<input type='text' id='ssid' name='ssid' required placeholder='Type network name'>"
"<select id='ssid_select' style='display:none' onchange=\"document.getElementById('ssid').value=this.value;if(this.value==='__manual__'){document.getElementById('ssid').style.display='block';document.getElementById('ssid').value='';document.getElementById('ssid').focus();}else{document.getElementById('ssid').style.display='none';}\"></select>"
...
"</div>"
"<script>"
"function scanNetworks(){var btn=document.getElementById('scan-btn');var loading=document.getElementById('scan-loading');var sel=document.getElementById('ssid_select');var inp=document.getElementById('ssid');btn.disabled=true;loading.style.display='block';fetch('/scan_networks').then(r=>r.json()).then(networks=>{sel.innerHTML='';if(networks.length>0){var ph=document.createElement('option');ph.value='';ph.text='Select a network';sel.appendChild(ph);networks.forEach(net=>{var opt=document.createElement('option');opt.value=net.ssid;opt.text=net.ssid;sel.appendChild(opt)});var manual=document.createElement('option');manual.value='__manual__';manual.text='Enter manually...';sel.appendChild(manual);sel.style.display='block';inp.style.display='none';}else{sel.style.display='none';inp.style.display='block';}loading.style.display='none';btn.disabled=false}).catch(e=>{console.error('Scan failed:',e);loading.style.display='none';btn.disabled=false;sel.style.display='none';inp.style.display='block'})}"
