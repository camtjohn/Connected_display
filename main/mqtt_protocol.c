...
int mqtt_protocol_parse_shared_view_frame(const uint8_t *payload, uint8_t payload_len,
    ...
    // Parse seq (network byte order: big-endian)
    frame->seq = (payload[0] << 8) | payload[1];
    
    memcpy(frame->red, payload + 2, sizeof(frame->red));
...
int mqtt_protocol_build_shared_view_updates(const mqtt_shared_pixel_update_t *updates, uint8_t num_updates,
                                            uint16_t seq,
                                            uint8_t *buffer, uint8_t buffer_size) {
    if (updates == NULL || buffer == NULL || num_updates == 0) {
        ESP_LOGE(TAG, "Invalid params for build_shared_view_updates");
        return -1;
    }

    uint8_t payload_len = 3 + (num_updates * 3);  // seq (2) + count (1) + triplets
    uint8_t total_len = MQTT_PROTOCOL_HEADER_SIZE + payload_len;
    if (buffer_size < total_len) {
        ESP_LOGE(TAG, "Buffer too small for shared view updates: need %d, have %d", total_len, buffer_size);
        return -1;
    }

    buffer[0] = MSG_TYPE_SHARED_VIEW_UPDATES;
    buffer[1] = payload_len;
    
    // Encode seq in network byte order (big-endian)
    buffer[2] = (seq >> 8) & 0xFF;
    buffer[3] = seq & 0xFF;
    buffer[4] = num_updates;

    uint8_t idx = 5;
    for (uint8_t i = 0; i < num_updates; i++) {
        buffer[idx++] = updates[i].row;
        buffer[idx++] = updates[i].col;
        buffer[idx++] = updates[i].color;
    }
...
int mqtt_protocol_parse_shared_view_updates(const uint8_t *payload, uint8_t payload_len,
    ...
    if (payload == NULL || updates == NULL || out_count == NULL || out_seq == NULL) {
        ESP_LOGE(TAG, "Invalid params for parse_shared_view_updates");
        return -1;
    }

    if (payload_len < 3) {
        ESP_LOGE(TAG, "Shared view updates payload too short");
        return -1;
    }

    // Parse seq (network byte order: big-endian)
    *out_seq = (payload[0] << 8) | payload[1];
    uint8_t count = payload[2];
    
    uint8_t expected_len = 3 + (count * 3);
    if (payload_len < expected_len) {
        ESP_LOGE(TAG, "Shared view updates length mismatch: %d vs expected %d", payload_len, expected_len);
        return -1;
    }

    if (count > max_updates) {
        ESP_LOGW(TAG, "Too many updates (%d), truncating to %d", count, max_updates);
        count = max_updates;
    }

    uint8_t idx = 3;
    for (uint8_t i = 0; i < count; i++) {
        updates[i].row = payload[idx++];
        updates[i].col = payload[idx++];
        updates[i].color = payload[idx++];
    }
